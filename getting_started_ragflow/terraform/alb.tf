/*
============================================================================
Application Load Balancer for RAGFlow
============================================================================
Public ALB to expose RAGFlow
*/
...
locals {
  ...
  ragflow_url           = local.alb_dns_enabled ? "https://${var.alb_domain_name}" : "http://${aws_lb.alb.dns_name}"
}
...
resource "aws_lb_target_group" "alb_ragflow" {
  name        = "${local.name_prefix}-ragflow"
  port        = local.ragflow_port
  protocol    = "HTTP"
  vpc_id      = module.vpc.vpc_id
  target_type = "ip"
  health_check {
    # Cheap liveness probe. The deeper check (database, document engine, cache,
    # object store) is the container health check on /api/v1/system/healthz,
    # which ECS uses to replace an unhealthy task.
    path     = "/api/v1/system/ping"
    interval = 30
  }
  # RAGFlow keeps the login token client-side, but chat streaming benefits from
  # a stable task when the service is scaled out.
  stickiness {
    type    = "lb_cookie"
    enabled = true
  }
...
resource "aws_lb_listener" "alb" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.alb_ragflow.arn
  }
}
...
output "ragflow_url" {
  description = "RAGFlow URL (HTTPS when a custom domain is configured)"
  value       = local.ragflow_url
}
