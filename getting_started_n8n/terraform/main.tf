/*
============================================================================
stdapi.ai with n8n Deployment
============================================================================
This configuration deploys stdapi.ai as a backend service (no ALB) with n8n
as the workflow-automation frontend accessible via ALB, preconfigured with a
credential and a set of runnable sample workflows.

Features:
- stdapi.ai as internal OpenAI-compatible and Anthropic-compatible backend
- n8n with a public HTTPS-only ALB
- Aurora PostgreSQL as n8n's database
- Preconfigured stdapi.ai credential and sample workflows, seeded at first start
...
locals {
  ...
  first plan, which keeps a single `tofu apply` working on an empty state.
  */
...
locals {
  ...
  stdapi_anthropic_api_url = "${local.stdapi_url}/anthropic"
