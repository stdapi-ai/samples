/*
============================================================================
Application Load Balancer for LobeHub
============================================================================
Public ALB to expose LobeHub
*/
...
locals {
  ...
  lobehub_url           = local.alb_dns_enabled ? "https://${var.alb_domain_name}" : "http://${aws_lb.alb.dns_name}"
}
...
resource "aws_lb_target_group" "alb_lobehub" {
  name        = "${local.name_prefix}-lobehub"
  port        = local.lobehub_port
  protocol    = "HTTP"
  vpc_id      = module.vpc.vpc_id
  target_type = "ip"
  health_check {
    # Lightweight, dependency-free route (no DB/Redis/S3 check) so a
    # transient backing-service hiccup doesn't pull every task out of the ALB.
    path = "/api/version"
  }
  # LobeHub authenticates via cookie-backed sessions; with the default 2-task
  # autoscaling floor (see lobehub.tf), pin a browser to one task so it keeps
  # its session.
  stickiness {
    type    = "lb_cookie"
    enabled = true
...
resource "aws_lb_listener" "alb" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.alb_lobehub.arn
  }
}
...
output "lobehub_url" {
  description = "LobeHub URL (HTTPS when a custom domain is configured)"
  value       = local.lobehub_url
}
